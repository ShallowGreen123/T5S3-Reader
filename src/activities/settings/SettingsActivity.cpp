...
void SettingsActivity::rebuildSettingsLists() {
  ...
  systemSettings.push_back(SettingInfo::Action(StrId::STR_BATTERY_STATUS, SettingAction::BatteryStatus));
...
void SettingsActivity::toggleCurrentSetting() {
  ...
  if (setting.type == SettingType::TOGGLE && setting.valuePtr != nullptr) {
    ...
    const uint8_t optionCount = setting.enumStringValues.empty() ? static_cast<uint8_t>(setting.enumValues.size())
...
bool SettingsActivity::onTouchTap(int16_t x, int16_t y) {
  ...
  if (y >= tabTop && y < tabBottom) {
    const int tappedCategory = std::clamp((x * categoryCount) / std::max(1, pageWidth), 0, categoryCount - 1);
    if (tappedCategory != selectedCategoryIndex) {
  ...
  const int touchedSetting = pageStartIndex + row;
  if (row < 0 || row >= pageItems || touchedSetting < 0 || touchedSetting >= settingsCount) {
    return false;
  }
